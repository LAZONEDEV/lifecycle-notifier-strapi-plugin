...
import { collections, interceptors, planetCollection, recipients } from './fakeData';
...
describe('test suite for SubscriptionDialog component', () => {
  ...
  it('should call SubscriptionService.create when submitting a new subscription', async () => {
    ...
    // select collection
    await selectOption(fieldLabels.collection, planetCollection.info?.name!, getByRole);

    // select recipients
    await selectOption(
      fieldLabels.recipients,
      createRecipientLabel(testInputs.recipients[0], planetCollection.info?.name!),
      getByRole
    );
    ...
    // select relations
    await selectOption(fieldLabels.relations, testInputs?.relations?.[0], getByRole);

    // select interceptors
    await selectOption(fieldLabels.interceptors, testInputs?.interceptors?.[0], getByRole);
    ...
    expect(SubscriptionService.create as jest.Mock).toHaveBeenCalledWith(testInputs);
    expect(onClose).toHaveBeenCalled();
  });

  it('should fill all fields with the provided value for edition', async () => {
    const onClose = jest.fn();
    const { getByRole } = render(<SubscriptionDialog editing={testInputs} onClose={onClose} />, {
      wrapper: StrapiThemeProvider,
    });

    expect(getByRole('textbox', { name: fieldLabels.subject })).toHaveValue(testInputs.subject);

    expect(getByRole('textbox', { name: fieldLabels.template })).toHaveValue(testInputs.content);

    await waitFor(() => {
      expect(getByRole('combobox', { name: fieldLabels.event })).toHaveTextContent(
        testInputs.eventType
      );

      expect(getByRole('combobox', { name: fieldLabels.collection })).toHaveTextContent(
        planetCollection.info?.name!
      );

      expect(getByRole('combobox', { name: fieldLabels.recipients })).toHaveTextContent(
        createRecipientLabel(testInputs.recipients[0], planetCollection.info?.name!)
      );

      expect(getByRole('combobox', { name: fieldLabels.media })).toHaveTextContent(
        testInputs.mediaFields[0]
      );

      expect(getByRole('combobox', { name: fieldLabels.relations })).toHaveTextContent(
        testInputs?.relations?.[0]!
      );

      expect(getByRole('combobox', { name: fieldLabels.interceptors })).toHaveTextContent(
  ...
  it('should call SubscriptionService.update when submitting an edited subscription', async () => {
    ...
    const { getByRole } = render(
      <SubscriptionDialog editing={{ ...testInputs, id: testSubId }} onClose={onClose} />,
      {
        wrapper: StrapiThemeProvider,
      }
    );
    ...
    expect(SubscriptionService.update as jest.Mock).toHaveBeenCalledWith(testSubId, {
      ...testInputs,
      subject: newSubject,
      id: testSubId,
    });
