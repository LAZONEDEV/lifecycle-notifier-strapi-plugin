...
describe('Unit test for listenedCollectionUpdater', () => {
  ...
  it('should register a new listened collection', async () => {
    ...
    expect(listenedCollection.add).toHaveBeenCalledWith(event.params.data.collectionName);
  });
  ...
  it('should remove a collection item from listened collection', async () => {
    ...
    (
      getSubscriptionsForCollection as jest.Mock<() => Promise<SubscriptionEntry[]>>
    ).mockResolvedValue([]);

    await updateListenedCollectionOnChangeOnSubsCollection(event);

    expect(getSubscriptionsForCollection).toHaveBeenCalledWith(event.params.data.collectionName);
    expect(listenedCollection.delete).toHaveBeenCalledWith(event.params.data.collectionName);
