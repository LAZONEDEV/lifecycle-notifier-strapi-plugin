...
export const getInterceptorsMap = (): InterceptorsMap | undefined => {
  const availableInterceptors = getPluginConfig(ConfigKeys.INTERCEPTORS) as Interceptor[];
...
export const applyInterceptors = async (
  ...
  for (const interceptorName of interceptorNames) {
    ...
    try {
      const interceptorOutput = await interceptorsMap[interceptorName](generatedData);
      ...
      console.error(`The error below occurred while applying the '${interceptorName}' interceptor`);
      console.error(error);
      throw error;
    }
  }
