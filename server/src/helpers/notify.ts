...
export const notify = async (
  ...
  const mailTo = await getRecipientEmail(recipient, entry, subscription.collectionName);
  if (mailTo) {
    ...
    if (subscription.mediaFields?.length) {
      ...
      if (mediasToLoadIds.length) {
        try {
          ...
          console.error('The error below occurred while loading the media files from the database');
          console.error(error);
        }
      }
