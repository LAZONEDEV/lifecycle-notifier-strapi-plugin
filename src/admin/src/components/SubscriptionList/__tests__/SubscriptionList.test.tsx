...
const subList = [
  {
    ...
    content: 'The planet <%= name %> has been discovered on <%= formattedDiscoveryDate %>!',
    mediaFields: ['image'],
    relations: [],
    interceptors: [],
  },
...
describe('suite test for SubscriptionList component', () => {
  ...
  it('should render correctly', () => {
    const { getByText } = render(
      <SubscriptionList onEdit={() => {}} subList={subList} loadSubs={() => {}} />,
      {
  ...
  it('should call onEdit when edit button is clicked', async () => {
    ...
    const { getByTestId } = render(
      <SubscriptionList onEdit={onEditMock} subList={subList} loadSubs={() => {}} />,
      {
        wrapper: StrapiThemeProvider,
      }
    );
  ...
  it('should open modal and call SubscriptionService.delete when trying to confirm a subscription to be deleted', async () => {
    ...
    expect(SubscriptionService.delete as jest.Mock).toHaveBeenCalledWith(subList[0].id);
  });
