...
describe('getEntryWithRelation', () => {
  ...
  it('should return entry with relations successfully', async () => {
    const mockEntityManager = {
      findOne: jest.fn<() => Promise<CollectionEntry>>().mockResolvedValue(mockResult),
    };
    (getCollectionEntityManager as jest.Mock).mockReturnValue(mockEntityManager);
  ...
  it('should log error if an exception occurs', async () => {
    ...
    await getEntryWithRelation(mockCollectionUid, mockEntry, mockRelationsToPopulate);
